...
use crate::constants::*;
use crate::processor::orca::OrcaSwap;
use crate::state::*;
use solana_program::{
...
pub fn handle_swap_orca(
    ctx: &Context<SwapOrcaAccount>,
    amount_in: u64,
...
pub fn handle_swap_to_solana_orca(
    ...
    let whirl_pool = ctx.accounts.whirl_pool.to_account_info();
    let token_owner_account_a = ctx.accounts.token_owner_account_a.to_account_info();
    let token_vault_a = ctx.accounts.token_vault_a.to_account_info();
    let token_owner_account_b = ctx.accounts.token_owner_account_b.to_account_info();
    let token_vault_b = ctx.accounts.token_vault_b.to_account_info();
    let tick_array_0 = ctx.accounts.tick_array_0.to_account_info();
    let tick_array_1 = ctx.accounts.tick_array_1.to_account_info();
    let tick_array_2 = ctx.accounts.tick_array_2.to_account_info();
    ...
    let swap_accounts = vec![
        ...
        AccountMeta::new(*token_authority.key, false),
        ...
        AccountMeta::new(*token_vault_b.key, false), // amm_target
        AccountMeta::new_readonly(*tick_array_0.key, false),
